...
class KDTree
{
  ...
  private:
     ...
     */
    int partition(vector<Point<Dim>> &list, int left, int right, int pivotIndex, int curDim);
    Point<Dim> select(vector<Point<Dim>> &list, int left, int right, int k, int curDim);
    KDTreeNode* createNode(vector<Point<Dim>> &subList, int curDim);
    Point<Dim> findBest(Point<Dim> target, KDTreeNode* curr, int curDim) const;
    KDTreeNode* copyHelper(KDTreeNode *subRoot);
    void del(KDTreeNode *node);
};
